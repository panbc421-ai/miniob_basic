...
RC UpdateExecutor::execute(SQLStageEvent *sql_event)
{
  ...
  for (size_t i = 0; i < units.size(); i++) {
    for (int f = sys_field_num; f < field_num; f++) {
      ...
      if (fm == units[i].field_meta || 0 == strcmp(fm->name(), units[i].field_meta->name())) {
  ...
  while (scanner.has_next()) {
    ...
    if (filter_stmt != nullptr) {
      ...
      bool match = false;
      rc = FilterStmt::eval(filter_stmt, row_tuple, match);
      if (rc != RC::SUCCESS) {
        scanner.close_scan();
        return rc;
      }
  ...
  for (auto &[rid, data_copy] : records_to_update) {
    Record old_record;
    old_record.set_data(data_copy.data(), static_cast<int>(data_copy.size()));
