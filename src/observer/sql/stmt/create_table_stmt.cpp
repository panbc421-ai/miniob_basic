...
RC CreateTableStmt::create(Db *db, CreateTableSqlNode &create_table, Stmt *&stmt)
{
  ...
  if (create_table.is_ctas && create_table.select_sql != nullptr) {
    stmt = new CreateTableStmt(create_table.relation_name, create_table.attr_infos, std::move(*create_table.select_sql));
    delete create_table.select_sql;
    create_table.select_sql = nullptr;
    sql_debug("create table as select statement: table name %s", create_table.relation_name.c_str());
    return RC::SUCCESS;
  }
