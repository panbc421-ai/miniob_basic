...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  std::vector<std::string> attr_names = create_index.attribute_names;
  if (attr_names.empty() && !create_index.attribute_name.empty()) {
    attr_names.push_back(create_index.attribute_name);
  }
  if (attr_names.empty()) {
    LOG_WARN("invalid argument: no index fields. table=%s", table_name);
    return RC::INVALID_ARGUMENT;
  }
  ...
  field_metas.reserve(attr_names.size());
  for (const std::string &attr_name : attr_names) {
    if (is_blank(attr_name.c_str())) {
      LOG_WARN("invalid index field name. table=%s", table_name);
      return RC::INVALID_ARGUMENT;
    }
