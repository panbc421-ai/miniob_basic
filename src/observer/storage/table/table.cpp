...
RC Table::make_record(int value_num, const Value *values, Record &record, const char *forced_null_fields)
{
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (field->type() == TEXTS && value.length() > MAX_TEXT_VALUE_LENGTH) {
      LOG_WARN("text value is too long. table=%s, field=%s, value_len=%d, field_len=%d",
               table_meta_.name(), field->name(), static_cast<int>(value.length()), MAX_TEXT_VALUE_LENGTH);
      return RC::SCHEMA_FIELD_TYPE_MISMATCH;
    }
  }
  ...
  while (scanner.has_next()) {
    ...
    if (is_unique) {
      ...
      IndexScanner *key_scanner = index->create_scanner(
          key.data(), index->key_length(), true, key.data(), index->key_length(), true);
