...
| (待提交) | update-select | UPDATE 支持多列 SET（`SET a=v, b=v WHERE ...`）及 `col=(标量子查询)`；UpdateSqlNode 改为 assignments 列表，executor 用 make_record 重建行（正确处理 NULL/char 填充/索引） | ✅ 多列只改命中行、单列回归正常、子查询取值正确 |
